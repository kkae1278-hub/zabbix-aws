...
- Zabbix Server の user_data では `-e` を外して `set -uo pipefail` にする
- 失敗しても継続したいコマンドには `|| true` を付ける

```bash
# 失敗しても続行する場合
apt-get install -y zabbix-agent2 || true
...
`db.t3.micro` は RDS Enhanced Monitoring と Performance Insights に非対応。
